...
@onready var opponent_hand: Hand = $Panel/CardManager/OpponentHand
...
@export var card_fade_in_duration: float = 0.3  # How long each card takes to fade in
@export var win_condition_delay: float = 1.0  # Delay after last card before showing win/loss
@export var score_count_duration: float = 1.0  # How long the winner's score counts up
@export var final_score_delay: float = 0.5  # Delay before showing loser's score
...
## Shows the round total screen and begins the card reveal sequence
func show_round_results(p_cards: Array, opp_cards: Array, p_previous_score: int, opp_previous_score: int) -> void:
	if is_animating:
		return
...
func _reveal_card_at_index(index: int) -> void:
	...
	if index < player_cards.size():
		player_card = player_cards[index]
		if player_hand:
	...
	if index < opponent_cards.size():
		opponent_card = opponent_cards[index]
		if opponent_hand:
	...
	# Update totals with card values
	if player_card and "card_value" in player_card:
		player_round_total += player_card.card_value
		_animate_total_update(player_total_label, player_round_total)
	
	if opponent_card and "card_value" in opponent_card:
		opponent_round_total += opponent_card.card_value
