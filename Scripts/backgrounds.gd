...
func _choose_random_background(bg_node: Node) -> void:
	...
	picked.visible = true
	# Force full-rect anchors and expansion so the TextureRect covers its parent
	# Anchors preset 15 corresponds to full rect in exported scenes
	if picked is Control:
		picked.anchors_preset = 15
		picked.expand = true
		# Prefer cover so it fills the area without leaving borders (might crop); fallback to scale if you want distortion instead
		picked.stretch_mode = 5
