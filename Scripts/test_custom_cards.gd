...
func _ready():
	LOG.log("Testing custom card system...")
	_create_test_cards()
...
func _create_test_cards():
	...
	for i in range(test_cards.size()):
		...
		if card:
			LOG.log_args(["Successfully created card:", card_name])
			# Show back face for even-indexed cards (every other card)
			if i % 2 == 1:
				card.show_front = false
				LOG.log_args(["Set card to show back:", card_name])
			else:
				LOG.log_args(["Card showing front:", card_name])
		else:
			LOG.log_args(["Failed to create card:", card_name])
