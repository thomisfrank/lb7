...
var unique_id: int
var drop_zone_scene = preload("res://Scenes/CardFramework/Containers/drop_zone.tscn")
...
func _ready() -> void:
	...
	# Find nearest CardManager in the ancestor chain so containers can be placed
	# under other nodes (e.g., PlayArea) but still register with the CardManager.
	var ancestor = get_parent()
	var found_manager: CardManager = null
	while ancestor != null:
		if ancestor is CardManager:
			found_manager = ancestor
			break
		ancestor = ancestor.get_parent()

	if found_manager != null:
		card_manager = found_manager
	else:
		# Fallback: sometimes the CardManager is not an ancestor but a sibling
		# (for example in main.tscn CardManager and PlayArea are both children of GameLayer).
		# Search the scene root for a node named "CardManager" and use it if present.
		var root = get_tree().get_root()
		# Some root types (Window) don't implement find_node; do a safe DFS search.
		var stack: Array = [root]
		var maybe_cm: CardManager = null
		while stack.size() > 0:
			var node = stack.pop_back()
			if node is CardManager:
				maybe_cm = node
				break
			# push children for search
			for child in node.get_children():
				stack.push_back(child)
		if maybe_cm != null:
			card_manager = maybe_cm
		else:
...
func check_card_can_be_dropped(cards: Array) -> bool:
	# Debug logging for PlayArea
	var is_play_area = (self.name == "PlayArea")
	
	if not enable_drop_zone:
		if is_play_area:
			LOG.log("PlayArea: check_card_can_be_dropped FAILED - enable_drop_zone is false")
		return false

	if drop_zone == null:
		if is_play_area:
			LOG.log("PlayArea: check_card_can_be_dropped FAILED - drop_zone is null")
		return false

	if drop_zone.accept_types.has(CardManager.CARD_ACCEPT_TYPE) == false:
		if is_play_area:
			LOG.log("PlayArea: check_card_can_be_dropped FAILED - accept_types doesn't have CARD_ACCEPT_TYPE")
		return false
		
	if not drop_zone.check_mouse_is_in_drop_zone():
		if is_play_area:
			LOG.log("PlayArea: check_card_can_be_dropped FAILED - mouse not in drop zone")
		return false
	
	var can_add = _card_can_be_added(cards)
	if is_play_area:
		LOG.log_args(["PlayArea: check_card_can_be_dropped - _card_can_be_added returned:", can_add])
