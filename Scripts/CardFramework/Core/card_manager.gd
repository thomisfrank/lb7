...
func _ready() -> void:
	...
	LOG.log_args(["CardManager: Factory initialized with card_info_dir:", card_factory.card_info_dir])
	LOG.log_args(["CardManager: Factory initialized with card_asset_dir:", card_factory.card_asset_dir])
...
func _pre_process_exported_variables() -> bool:
	# If the exported PackedScene is not set (broken ext_resource or manual edit),
	# try to find an existing CardFactory node child or elsewhere in the scene tree
	# before failing. This helps when scene resource UIDs are invalid but the
	# factory is already present in the scene.
	if card_factory_scene == null:
		# Check for an existing CardFactory child
		for child in get_children():
			if child is CardFactory:
				card_factory = child
				LOG.log("CardManager: Using existing CardFactory child instance.")
				return true

		# Search the entire scene for a CardFactory instance (fallback)
		var root = get_tree().get_root()
		var stack: Array = [root]
		while stack.size() > 0:
			var node = stack.pop_back()
			if node is CardFactory:
				card_factory = node
				LOG.log("CardManager: Found CardFactory instance in scene tree and will use it.")
				return true
			for c in node.get_children():
				stack.push_back(c)

		# If nothing found, show error as before
		push_error("CardFactory is not assigned! Please set it in the CardManager Inspector.")
		return false

	# If a PackedScene was provided, instantiate it
