...
## Horizontal offset applied per-card (pixels). Use to create a slight "3D" shear
## when cards are stacked. Positive values shift subsequent cards right; for
## LEFT layout the offset is applied to the left (negative X).
@export var stack_horizontal_offset: float = 0.0
...
func _calculate_offset(index: int) -> Vector2:
	...
	# Apply optional horizontal shear/offset to give a 3D stacked appearance.
	# For LEFT layout we apply the offset to the left; otherwise to the right.
	if stack_horizontal_offset != 0.0:
		if layout == PileDirection.LEFT:
			offset.x -= actual_index * stack_horizontal_offset
		else:
			offset.x += actual_index * stack_horizontal_offset
