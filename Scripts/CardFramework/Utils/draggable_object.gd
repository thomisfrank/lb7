...
@export var moving_speed: float = CardFrameworkSettings.ANIMATION_MOVE_SPEED
...
@export var hover_distance: float = CardFrameworkSettings.PHYSICS_HOVER_DISTANCE
...
func move(_target_destination: Vector2, degree: float) -> void:
	# Skip if current position and rotation match target
	if global_position == _target_destination and rotation == degree:
		return
	...
	self.target_destination = _target_destination
