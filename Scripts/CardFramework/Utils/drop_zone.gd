...
func init(_parent: Node, _accept_types: Array =[]):
	parent = _parent
	self.accept_types = _accept_types
...
func check_mouse_is_in_drop_zone() -> bool:
	...
	var rect = sensor.get_global_rect()
	var result = rect.has_point(mouse_position)
	
	# Debug logging for PlayArea specifically (muted unless LOG enabled)
	if parent and parent.name == "PlayArea":
		LOG.log_args(["PlayArea DropZone check: mouse=", mouse_position, " rect=", rect, " result=", result])
